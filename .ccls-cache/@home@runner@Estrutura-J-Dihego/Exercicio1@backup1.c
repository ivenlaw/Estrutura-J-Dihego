...
void quebraLinha (int tamLinha, char linha[], int iS, char vString[][iS], int vInteiro [], float vFloat [], float vPontosx [], float vPontosy [], int iP, int iI,  int iF);
...
//  char *vString=malloc(TAM_MAX_LINHA * sizeof(char)); 
  ...
  while (fgets(linha, TAM_MAX_LINHA, arquivoEntrada)!=NULL) {
    quebraLinha (tamLinha, linha, iS, vString, vInteiro , vFloat, vPontosx, vPontosy, iF, iP, iI);  
...
void trataString (int iS, char vString[][iS]){
  ...
  int y=sizeof(vString[iS][iS]);
  if(iS>0){
    while(iS>0) {
      for(int i=iS-1; i<iS; i++){
        printf("string anterior= %s e i= %d\n", &vString[i][i], iS-1);
        sscanf(&vString[i][i], "%s", provisorio);
//          strcpy(provisorio, &vString[a]);
        printf("provisorio %s\n", provisorio);
      }
      if((strcmp(&vString[iS][iS], provisorio)<0)){
        for(int i=iS-1; i<iS; i++){
          sscanf(&vString[iS][iS], "%s", &vString[iS][i]);
  //          strcpy(&vString[a], &vString[iS]);
          printf("strin anerior pos mud %s\n", &vString[i][i]);
        }
        sscanf(provisorio, "%s", &vString[iS][iS]);
...
void impressaoString (int iS, char vString[][iS], char sString[]){
  ...
  strcpy(sString, &vString[0][0]);

  while (i<y-1){
    strcat(sString, " "); 
    strcat(sString, &vString[i][i]);  
    printf("sSprint é %s\n", sString);
    i++;
    }
